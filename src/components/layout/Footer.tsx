...
const FOOTER_SECTIONS = [
  ...
  { title: "Правила и настройки", links: [
...
export default function Footer() {
  ...
  return (
    <footer style={{ background: "#fff", borderTop: "1px solid var(--booking-border)", marginTop: 16 }}>
      <div className="site-container" style={{ paddingTop: 32, paddingBottom: 24 }}>
        {/* Bottom links row */}
        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 0", marginBottom: 24, fontSize: 13, color: "#595959", borderBottom: "1px solid var(--booking-border)", paddingBottom: 20 }}>
          {BOTTOM_LINKS.map((link, i) => (
            <span key={i}>
              <a href={link.path} onClick={(e) => handleLink(e, link.path)}
                style={{ color: "#595959", textDecoration: "none" }}
                onMouseEnter={e => (e.currentTarget.style.textDecoration = "underline")}
                onMouseLeave={e => (e.currentTarget.style.textDecoration = "none")}
              >{link.label}</a>
              {i < BOTTOM_LINKS.length - 1 && <span style={{ margin: "0 8px", color: "#ccc" }}>·</span>}
            </span>
          ))}
        </div>
            ...
            <div key={i}>
              ...
              <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 8 }}>
                {section.links.map((link, j) => (
                  <li key={j}>
                    <a href={link.path} onClick={(e) => handleLink(e, link.path)}
                      style={{ color: "var(--booking-blue-light)", textDecoration: "none", fontSize: 13, lineHeight: 1.4, cursor: "pointer" }}
                      onMouseEnter={e => (e.currentTarget.style.textDecoration = "underline")}
                      onMouseLeave={e => (e.currentTarget.style.textDecoration = "none")}
                    >{link.label}</a>
                  </li>
      ...
      <div style={{ borderTop: "1px solid var(--booking-border)" }}>
        <div className="site-container" style={{ paddingTop: 16, paddingBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            <img src="https://flagcdn.com/20x15/uz.png" alt="UZ" style={{ width: 24, borderRadius: 2 }} />
            <span style={{ fontSize: 14, fontWeight: 600 }}>UZS</span>
          </div>
          <div style={{ color: "var(--booking-text-light)", fontSize: 12, marginBottom: 12 }}>
            Booking.com — часть Booking Holdings Inc., мирового лидера в сфере онлайн-туризма и сопутствующих услуг.<br />
            Copyright © 1996–2026 Booking.com™. Все права защищены.
          </div>
          <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
            {["Booking.com", "priceline", "KAYAK", "agoda", "OpenTable"].map((brand, i) => (
              <span key={i} style={{ fontSize: 13, fontWeight: 700, color: i === 0 ? "var(--booking-blue)" : "var(--booking-text-light)" }}>
                {brand}
              </span>
            ))}
          </div>
